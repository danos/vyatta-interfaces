...
sub new {
    ...
    if ( index( $name, "vrrp" ) != -1 ) {
        ...
        # Vrrp interface names do not contain information that allow us
        # to map back to the parent interface nor the group number that
        # the interface is associated with. We therefore read in a file
        # that's generated by vrrp to determine this information.

        my $vrrp_intf_map = "/var/run/vrrpd/vrrp_intf.map";

        if ( -e $vrrp_intf_map ) {

            my @lines = read_file($vrrp_intf_map);
            die "Error: read $!" unless (@lines);    # unless not empty
            ...
            for ( my $i = 0 ; $i < ( scalar @lines ) ; $i++ ) {
                ...
                # could optimize search if file was ordered
            ...
            # example mapping: dp0vrrp1=dp0s3v7
            if ( $index != -1 ) {
                my $line = $lines[$index];
